import { formatBookingDateTime } from "./booking-core.js";

const BUSINESS_NAME = "Cleveland Clean";
...
const adminHtml = (booking) => {
  ...
  const replySubject = encodeURIComponent(`Re: Cleveland Clean booking request for ${formattedDateTime}`);
  ...
  const rows = [
    ...
    ["Service", booking.service],
    ["Vehicle", booking.vehicle],
    ["Requested time", formattedDateTime],
    ["Address", booking.address],
    ["City", booking.city],
    ["Status", booking.status],
...
const receivedHtml = (booking) => `
  <div style="font-family:Arial,sans-serif;line-height:1.5;color:#101317">
    ...
    <p><strong>Requested time:</strong> ${escapeHtml(formatBookingDateTime(booking))}</p>
...
export const sendNewBookingEmails = async (env, booking) => {
  ...
  const subjectDate = booking.date ? ` for ${formatBookingDateTime(booking)}` : "";
...
const messageHtml = (booking, body) => `
  <div style="font-family:Arial,sans-serif;line-height:1.5;color:#101317">
    <h1 style="margin:0 0 14px;font-size:24px">Message from ${BUSINESS_NAME}</h1>
    <p>Hi ${escapeHtml(booking.name)},</p>
    <div style="white-space:pre-wrap;margin:16px 0">${escapeHtml(body)}</div>
    <p><strong>Booking:</strong> ${escapeHtml(booking.service)}</p>
    <p><strong>Requested time:</strong> ${escapeHtml(formatBookingDateTime(booking))}</p>
    <p>Reply to this email or call ${BUSINESS_PHONE} if you have questions.</p>
  </div>
`;

export const makeCustomerMessageSubject = (booking) => {
  return `${BUSINESS_NAME} - ${booking.service} booking`;
};

export const sendCustomerMessageEmail = async (env, booking, message) => {
  const businessReplyTo = env.BOOKING_REPLY_TO_EMAIL || BUSINESS_EMAIL;
  const subject = message.subject || makeCustomerMessageSubject(booking);
  ...
  await sendEmail(env, {
    ...
    html: messageHtml(booking, message.body)
  });
