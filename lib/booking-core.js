...
const makeBookingWallTimeDate = (dateParts, timeParts = { hour: 12, minute: 0 }) => {
  const localAsUtc = Date.UTC(
    dateParts.year,
    dateParts.month - 1,
    dateParts.day,
...
export const formatBookingDateTime = (bookingOrDate, maybeTime) => {
  ...
  if (!timeParts) {
    return dateFormatter.format(makeBookingWallTimeDate(dateParts));
  }

  return dateTimeFormatter.format(makeBookingWallTimeDate(dateParts, timeParts));
};
